pipeline {
    ...
    stages{
        stage('build project'){
            steps{
                git 'https://github.com/HrishikeshD714/star-agile-banking-finance_capstone1.git'
        ...
        stage('Building  docker image'){
            steps{
                script{
                    sh 'docker build -t hrishi105/capstone_banking:v1 .'
                    sh 'docker images'
                }
            }
        }
        stage('push to docker-hub'){
            steps{
                withCredentials([usernamePassword(credentialsId: 'docker-creds', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push hrishi105/capstone_banking:v1'
                }
            }
        }
